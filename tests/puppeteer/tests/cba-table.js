...
it("Hovering table row dispatches rowmouseover event with the rowId in details", async() =>
{
  const items = await prepopulatedItems();
...
it("Hovering out table row dispatches rowmouseout event with the rowId in details", async() =>
{
  const items = await prepopulatedItems();
